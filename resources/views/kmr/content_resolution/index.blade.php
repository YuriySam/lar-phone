...
<div>
  ...
  <tbody>
    ...
    @foreach($content_resolutions as $item)
        <tr>
                ...
                <th> <a href="{{route('kmr.content_resolution.show', $item->id)}}"> {{$item->title}}</a> </th>
                {{-- <td> <a href="{{route('admin.content_resolution.show', $item->id)}}"> {{$item->created_at}} </a></td>
                <td> <a href="{{route('admin.content_resolution.show', $item->id)}}"> {{$item->updated_at}}</a></td>
                <td> <a href="{{route('admin.content_resolution.show', $item->id)}}"> {{$item->deleted_at}}</a></td>
                 --}}
        </tr>
  ...
  @endforeach
   ...
   <div>
        <a href="{{route('kmr.content_resolution.create')}}" class="btn btn-primary mb-3">Додати</a>

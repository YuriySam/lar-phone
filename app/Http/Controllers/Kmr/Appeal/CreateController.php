...
class CreateController extends BaseController
{
    public function __invoke()
    {
        ...
        $executor_ophones = Ophone::distinct()->where('surname', '>', '')->orderBy('surname', 'asc')->get(); //Виконавець (executor_ophone_id)
        //$executor_branches = Branch::All(); //Структурний підрозділ (executor_branch_id)
        $executor_branches = Branch::distinct()->where('branch', '>', '')->orderBy('branch', 'asc')->get(); //Структурний підрозділ (executor_branch_id)
        ...
        //dd('CreateController', 'appeals ' , $appeals);
        return view('kmr.appeal.create', compact('appeals','streets','type_docums','question_docums', 'destination_offices','source1_offices', 'source2_offices', 'amount_docums', 'content_resolutions', 'avtor_resolutions', 'executor_ophones', 'executor_branches', 'title_implementations','validities'));
    }
}

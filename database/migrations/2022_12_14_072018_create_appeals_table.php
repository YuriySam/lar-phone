...
class CreateAppealsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('appeals', function (Blueprint $table) {
            ...
            $table->string(             'surname_reg', '50')->nullable();
            $table->string(             'name_reg', '50')->nullable();
            $table->string(             'patronymic', '50')->nullable();
            
            $table->bigInteger(         'street_id'                    )->default('1')->nullable(); //Адреса з довідника
            $table->string(             'street_txt', '250'            )->nullable(); //Адреса текст

            $table->string(             'phone_reg', '12'               )->nullable(); //Номер телефону заявника
            $table->bigInteger(         'type_docum_id'                 )->default('1')->nullable();   //Вид документа з довідника скарга/подяка/запит/пропозиція
            $table->string(             'type_docum_txt','150'          )->nullable();   //Вид документа текст
            $table->bigInteger(         'question_docum_id'             )->default('1')->nullable();   //Порушені питання тип
            ...
            $table->bigInteger(         'destination_office_id'         )->default('1')->nullable(); //Куди надійшло звернення
            ...
            $table->bigInteger(         'source1_office_id'             )->default('1')->nullable(); //Звідки одержано (1)
            ...
            $table->bigInteger(         'source2_office_id'             )->default('1')->nullable(); //Звідки одержано (2)
            ...
            $table->bigInteger(         'source3_office_id'             )->default('1'); //Звідки одержано (3)
            ...
            $table->bigInteger(         'amount_docum_id'               )->default('1')->nullable(); //первинне/повторне/дубль
            $table->string(             'amount_docum_txt','50'         )->nullable(); //первинне/повторне/дубль

            $table->bigInteger(         'content_resolution_id'         )->default('1')->nullable();// Зміст резолюції	розгляд відповідь/розгляд/ не за адресою/	content_resolution_id	tab 	resolution	
            ...
            $table->bigInteger(         'group_docum_id'                )->default('1')->nullable();// (Вид документа)	Група документа	group_docum_id	tab 	group_docum	
			$table->string(             'group_docum_txt','225'         )->nullable();// (Вид документа)	Група документа	group_docum_id	tab 	group_docum	
			
            $table->bigInteger(         'executor_ophone_id'            )->default('1')->nullable();// Виконавець	взяти з нашого довідника? Завідувача або начальника або заступника	ophone_id	tab 	ophone	executor
            ...
            $table->bigInteger(         'executor_branch_id'            )->default('1')->nullable();
            ...
            $table->bigInteger(         'title_implementation_id'       )->default('1');//Результат розгляду	Список з 6-ти варіантів?	title_Implementation	tab 	rezRoz
            ...
            $table->bigInteger(         'validity_id'                   )->default('1');//Обгрунтованість	Список з 6-ти варіантів?	Validity	tab 	Validity
            ...
            $table->bigInteger(         'user_id'                       )->default('1');//Хто заповнював строку
